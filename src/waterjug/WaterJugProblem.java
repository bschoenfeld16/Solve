...
public class WaterJugProblem extends Problem {
    ...
    public WaterJugProblem() {
        ...
        List<Move> waterJugMoves = new ArrayList<Move>();

        setCurrentState(new WaterJugState(0, 0));

        WaterJugMove fillX = new WaterJugMove("Fill Jug X");
        waterJugMoves.add(fillX);

        WaterJugMove fillY = new WaterJugMove("Fill Jug Y");
        waterJugMoves.add(fillY);

        WaterJugMove emptyX = new WaterJugMove("Empty Jug X");
        waterJugMoves.add(emptyX);

        WaterJugMove emptyY = new WaterJugMove("Empty Jug Y");
        waterJugMoves.add(emptyY);

        WaterJugMove transferXtoY = new WaterJugMove("Transfer Jug X to Jug Y");
        waterJugMoves.add(transferXtoY);

        WaterJugMove transferYtoX = new WaterJugMove("Transfer Jug Y to Jug X");
        waterJugMoves.add(transferYtoX);

        setMoves(waterJugMoves);
    ...
    public boolean success() {
        ...
        if(object instanceof WaterJugState) {
        WaterJugState currentState = (WaterJugState) object;
        return currentState.getAmountOfWaterX() == 2 || currentState.getAmountOfWaterY() == 2;
    ...
    public Canvas getCanvasState() {
        WaterJugCanvas waterJugCanvas = new WaterJugCanvas(this.getCurrentState());
        return waterJugCanvas;
    }
